...
local function load_org_files()
  ...
  for _, d in ipairs(C.org_directories or {}) do for _, f in ipairs(utils.get_org_files(d)) do add(want, f) end end
...
function S.collect()
  ...
  local out = {}
  for _, it in ipairs(uniq) do if valid[it.todo_state] then out[#out+1]=it end end
  return out
end
